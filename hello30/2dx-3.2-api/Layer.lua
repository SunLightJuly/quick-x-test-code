
--------------------------------
-- @module Layer
-- @extend Node
-- @parent_module cc

--------------------------------
-- @function [parent=#Layer] create 
-- @param self
-- @return Layer#Layer ret (return value: cc.Layer)
        
--------------------------------
-- @function [parent=#Layer] getDescription 
-- @param self
-- @return string#string ret (return value: string)


----------- weiq ---------------

--------------------------------
-- @function [parent=#Layer] onTouch 
-- @param self
-- @param #function listener

--------------------------------
-- @function [parent=#Layer] enableTouch 
-- @param self
-- @param #function enabled

--------------------------------
-- @function [parent=#Layer] onKeypad 
-- @param self
-- @param #function listener

--------------------------------
-- @function [parent=#Layer] enableKeypad 
-- @param self
-- @param #function enabled

--------------------------------
-- @function [parent=#Layer] onAccelerate 
-- @param self
-- @param #function listener

--------------------------------
-- @function [parent=#Layer] enableAccelerometer 
-- @param self
-- @param #function enabled

--------------------------------
-- @function [parent=#Layer] addKeypadEventListener 
-- @param self
-- @param #function listener

--------------------------------
-- @function [parent=#Layer] registerScriptKeypadHandler 
-- @param self
-- @param #function listener

--------------------------------
-- @function [parent=#Layer] addAccelerateEventListener 
-- @param self
-- @param #function listener

--------------------------------
-- @function [parent=#Layer] registerScriptAccelerateHandler 
-- @param self
-- @param #function listener


        
return nil
