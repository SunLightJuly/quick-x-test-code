...
------------------------------------------------------------------------------
local appkeys
--appkeys = require "appkeys"
if appkeys==nil then
    appkeys = {key=nil, sign='XT'}
end

------------------------------------------------------------------------------
--Load origin framework
------------------------------------------------------------------------------
CCLuaLoadChunksFromZIP("res/framework_precompiled.zip", appkeys.key, appkeys.sign)
...
--local server = "http://192.168.111.142:8088/"
...
local function getScaleValueForBackground(bgImg)
    local nWidthScale = display.width/bgImg:getContentSize().width
    local nHeightScal = display.height/bgImg:getContentSize().height
    if (nWidthScale > nHeightScal) then
        return nWidthScale
    else
        return nHeightScal
    end
...
function UpdateScene:ctor()
    ...
    --Only Show logo when UpdateScene is running
    --[[
    CCFileUtils:sharedFileUtils():addSearchPath("res/")
    display.addSpriteFramesWithFile(GAME_TEXTURE_DATA_FILENAME, GAME_TEXTURE_IMAGE_FILENAME)
    self.bg = display.newSprite("#gw_logo.png", display.cx, display.cy)
    self.bg:setScale(getScaleValueForBackground(self.bg))
...
function UpdateScene:onEnter()
              self.curListFile =  self.path.."flist"               
	self.fileList = nil
	...
	CCLuaLog("----------------------------------------UpdateScene:endProcess")
            ...
            if self.fileList and self.fileList.stage then
                ...
                if checkOK then
                    for i,v in ipairs(self.fileList.stage) do
                        if v.act=="load" then
                            CCLuaLoadChunksFromZIP(self.path..v.name, appkeys.key, appkeys.sign)
                        end
                    end
